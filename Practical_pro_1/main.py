...
def clean_lines(lines, threshold):
    ...
    while len(lines) > 0:
        mean = np.mean(slope) # 取平均值
        diff = [abs(s - mean) for s in slope] # 对所有直线的斜率与平均斜率作差，得到所有直线的误差量
        idx = np.argmax(diff) # 找到误差最大的直线的索引
...
def image_process(img):
    ...
    ret, img_th = cv2.threshold(img_blur, 160, 255, cv2.THRESH_BINARY)
    img_bit_add = cv2.bitwise_and(img_th,img_th,mask=mask_img)
    img_edge = cv2.Canny(img_bit_add, 200, 255)
    return img_edge

path = os.getcwd()
img_path = os.path.join(path, "Opencv_learn", "Practical_pro_1/pic/lane2.jpg")

vedio_path = os.path.join(path, "Opencv_learn", "Practical_pro_1", "video", "cv2_yellow_lane.mp4")
if __name__ == "__main__":
    frame = cv2.VideoCapture(vedio_path)
    ...
    while True:
        ...
        # print(left_lines, "\n")
        clean_lines(left_lines, 0.05)
        clean_lines(right_lines, 0.05)
